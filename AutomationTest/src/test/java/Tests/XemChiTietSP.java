...
public class XemChiTietSP {
    ...
    // CT-2: Check tên
    @Test
    public void CT2_Verify_Name() {
        WebElement p = getProducts().get(0);

        String name = p.findElement(productName).getText();

        clickSafe(p);

        String detailName = wait.until(
                ExpectedConditions.visibilityOfElementLocated(By.tagName("h1"))
        ).getText();

        Assert.assertEquals(detailName, name);
    }

    // CT-3: Check giá
    @Test
    public void CT3_Verify_Price() {
        WebElement p = getProducts().get(0);

        String price = p.findElement(productPrice).getText();

        clickSafe(p);

        String detailPrice = wait.until(
